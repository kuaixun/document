...
* type定义：*nav*
* 需求说明
  <br>1、app主要模块导航位，图标、链接、排序后台均可配，固定展示，用户不可移除。<br>2、根据内容默认显示一行（5个+1个更多），点更多可下拉展示更多隐藏模块，最多展示2行11个；<br>3、如导航模块数量小于7，则不显示更多，反之才显示；<br>4、新闻-进入app底部资讯tab“热点”界面；视频-进入app底部视频tab首界面；其他-进入app底部导航对应栏目界面
* 实现说明
* 字段定义
